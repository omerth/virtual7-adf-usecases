...
public class diff_match_patch {
        ...
        for (int d = 0; d < pattern.length(); d++) {
            ...
            for (int j = finish; j >= start; j--) {
                ...
                if (d == 0) {
                    ...
                    rd[j] =
                            ((rd[j + 1] << 1) | 1) & charMatch | (((last_rd[j + 1] | last_rd[j]) << 1) | 1) | last_rd[j +
                            1];
                }
                if ((rd[j] & matchmask) != 0) {
    ...
    protected void patch_addContext(Patch patch, String text) {
        ...
        while (text.indexOf(pattern) != text.lastIndexOf(pattern) &&
            ...
            pattern =
                    text.substring(Math.max(0, patch.start2 - padding), Math.min(text.length(), patch.start2 + patch.length1 +
                                                                                 padding));
        }
        ...
        // Add the suffix.
        String suffix =
            text.substring(patch.start2 + patch.length1, Math.min(text.length(), patch.start2 + patch.length1 +
                                                                  padding));
        if (suffix.length() != 0) {
    ...
    public Object[] patch_apply(LinkedList<Patch> patches, String text) {
        ...
        for (Patch aPatch : patches) {
            ...
            if (text1.length() > this.Match_MaxBits) {
                ...
                if (start_loc != -1) {
                    end_loc =
                            match_main(text, text1.substring(text1.length() - this.Match_MaxBits), expected_loc + text1.length() -
                                       this.Match_MaxBits);
            ...
            if (start_loc == -1) {
                ...
                if (text1.equals(text2)) {
                    ...
                    text =
text.substring(0, start_loc) + diff_text2(aPatch.diffs) + text.substring(start_loc + text1.length());
                } else {
                        ...
                        for (Diff aDiff : aPatch.diffs) {
                            if (aDiff.operation != Operation.EQUAL) {
                                ...
                                if (aDiff.operation == Operation.INSERT) {
                                    // Insertion
                                    text =
text.substring(0, start_loc + index2) + aDiff.text + text.substring(start_loc + index2);
                                } else if (aDiff.operation == Operation.DELETE) {
                                    // Deletion
                                    text =
text.substring(0, start_loc + index2) + text.substring(start_loc + diff_xIndex(diffs, index1 + aDiff.text.length()));
                                }
                            }
                            if (aDiff.operation != Operation.DELETE) {
            ...
            while (!bigpatch.diffs.isEmpty()) {
                ...
                while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin) {
                    ...
                    if (diff_type == Operation.INSERT) {
                        ...
                        diff_text =
                                diff_text.substring(0, Math.min(diff_text.length(), patch_size - patch.length1 - Patch_Margin));
                        patch.length1 += diff_text.length();
                        start1 += diff_text.length();
